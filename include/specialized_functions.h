...
namespace mtx_f {
    ...
    template<std::size_t ROWS, std::size_t COLS>
    MatrixType<Rational, ROWS, COLS> scalar(MatrixType<Rational, ROWS, COLS> &matrix, int scalar) {
        MatrixType<Rational, ROWS, COLS> result{};
        for (std::size_t i{0}; i < ROWS; ++i) {
            for (std::size_t j{0}; j < COLS; ++j) {
                result[i][j] = matrix[i][j] * scalar;
    ...
    template<std::size_t ROWS, std::size_t MID, std::size_t COLS>
    MatrixType<int, ROWS, COLS> product(MatrixType<int, ROWS, MID> &matrix, MatrixType<int, MID, COLS> &other) {
        MatrixType<int, ROWS, COLS> result{};
